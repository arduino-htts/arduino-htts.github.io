(function(w) {
    ...
    w.shareText = function(score) {
        ...
        SubmitResults();
        if (score <= 10) return '不要摆烂啊草！！！';
        if (score <= 50) return '硬起来啊草！！！';
        if (score <= 99) return '不会吧？？？尽全力啊baka！！！';
        if (score <= 149) return '这才是你该有的样子嘛~！';
        if (score <= 199) return '椰叶 菜菜 浇浇 QAQ';
        return '我在看着你哦OVO';
    }
    ...
    console.log("不修改，好嘛？乱传又有什么用呢？(ˉ▽ˉ；)...（对不起我只是给群友水水群用冒犯到您私密马赛！！！！）");
}) (window);
